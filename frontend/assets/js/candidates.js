...
function loadCandidates() {
	fetch("http://localhost:8080/api/candiate")
		.then((res) => res.json())
		.then((data) => {
...
document
	...
	?.addEventListener("submit", function (event) {
	  ...
	  // Validate email
		...
		fetch("http://localhost:8080/api/candiate/add", {
			method: "POST",
			headers: {
...
function updateCandidate(candidateId, newName, newParty) {
	fetch(`http://localhost:8080/api/candiate/update/${candidateId}`, {
		method: "PUT",
		headers: { "Content-Type": "application/json" },
		body: JSON.stringify({ name: newName, party: newParty }),
	})
...
function deleteCandidate(id) {
	Swal.fire({
		...
		if (result.isConfirmed) {
			fetch(`http://localhost:8080/api/candiate/delete/${id}`, {
				method: "DELETE",
			})
				.then(() => {
